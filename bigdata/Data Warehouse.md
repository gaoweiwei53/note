...
5) 移除组
6) 
## [虚拟机设置静态ip](https://blog.csdn.net/qq_40824474/article/details/82749007) [参考](https://www.cnblogs.com/yyee/p/12899953.html)
...
3) 创建分区表，防止后续的全表扫描
### 2. DWD层
1) 对于日志，要解析每条日志的字符串，将其转为字段。按照内容解析。
...
- DWS层以需求为驱动
# 调度
azkaban是根据每个shell脚本执行结束后的返回值判断是否执行成功。shell脚本执行成功返回0，不成功返回1.

所以它的缺陷是即使脚本里，不是最后执行的向表内插入数据的语句出现错误，整个调度流程也会继续执行。

解决办法：
1) 为每个表的导入数据脚本建一个脚本文件, 这样还可以并行执行
2) 在脚本最后写个判断语句，有其中一个表的语句没执行成功，就返回不成功
