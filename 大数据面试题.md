...
1) 客户端向ResourceManager申请一个Application，RM返回Application资源提交路径以及applicationid
2) 客户端提交job运行所需的资源，资源提交完毕后，再向RM申请运行ApplicationMaster
3) 在RM内部产生一个Task, RM会将多个客户端提交产生的Task放入任务队列里
4) 空闲的NodeManager获取Task，创建COntainer，启动ApplicationMaster，从资源路径上下载job资源
5) MR向ResourceManager申请运行Map Task容器，NodeManager获取Task，创建COntainer
6) ApplicationMaster发起程序启动脚本，Map Task完成后，AM会再次向RM申请容器运行Reduce Task，创建COntainer，运行Reduce Task
7) Reduce Task完成后，AM向RM申请注销释放相应资源
## 6. HDFS安全模式
