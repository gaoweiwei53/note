...
### 4. 容错机制  
Spark streaming, structured streaming, flink都支持checkpointing来进行容错。
#### spark streaming
在输入端，容错级别取决于Source的特性
...
通过**checkpointing**和**Write-Ahead Logs**实现了端到端的exactly-once容错保证。但是在exactly-once容错保证下，structured streaming能实现100ms的延迟。在Spark 2.3版本引入了**Continuous Processing**后, 在at-least-once保证下实现1ms的延迟  
