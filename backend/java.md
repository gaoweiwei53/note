...
> 看实现hash表、红黑树部分的代码
# Java 锁机制
在Java中，每个对象都有一把锁，这个锁存放在对象头中，锁中记录了当前对象被哪个线程锁占用。Java对象包含三个部分，对象头-实例数据-填充字节。对象头存放了对象的运行时信息，其中包括**Mark Word**和**Klass pointer**. Class point指向了当前对象类型所在方法区中的类型数据。Mark Word存储了很多和当前对象运行时的状态有关的数据，比如hashcode,锁状态标志。

每一个堆对象都指向一个对象头(object header)(64位系统中占96bit/128bit), 对象头包括两部分：
- **mark word**(64bit): 对象头的第一个字，包括hashcode，分代年龄，锁状态等
- **klass pointer**(开启指针压缩位32bit，否则为64bit)：对象头的第二个字，指向另一个描述源对象布局和表现行为的对象

![mark word](../resources/Lock.png)

锁状态包括四种
- 无锁：不对资源进行锁定，所有线程可以访问对象。
...
2) 使用CGlib库，实现`MethodInterceptor`接口，重写`intercept`方法
